...
enum PracticeMode { dailyMix, listening, hardWords }
...
class PracticePlanner {
  ...
  Future<PracticePlan> planDailyMix({int limit = 10}) async {
    ...
    if (dueCards.length < limit) {
      final recent = await _srsStore.getRecentlyLearned(
    ...
    final listeningItems = listeningSource
        .map(
          (c) => PracticeItem(
            id: c.cardId,
            unitId: c.unitId,
    ...
    final minutes = ((dueCards.length * 30 + listeningItems.length * 20) / 60)
        .ceil();
